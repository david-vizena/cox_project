...
class IncidentService:
    ...
    Fetches alerts from Datadog and creates/manages incidents.
    """
    ...
    def sync_datadog_alerts(self) -> List[Incident]:
        """
        Fetch active alerts from Datadog and create incidents for them.
        Returns list of newly created incidents.
        """
        # Check if we have a Datadog client
        ...
        for alert in alerts:
            # Check if we already have an incident for this Datadog alert
            existing = self.store.find_by_datadog_alert_id(alert['id'])

            if existing:
                # Already have this incident, skip it
                continue
